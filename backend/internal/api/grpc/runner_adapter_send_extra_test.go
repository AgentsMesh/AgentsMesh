...
func TestGRPCRunnerAdapter_SendSubscribeTerminal(t *testing.T) {
	...
	t.Run("runner not connected", func(t *testing.T) {
		err := adapter.SendSubscribeTerminal(999, "pod-1", "ws://relay", "session-1", "token", true, 100)
		require.Error(t, err)
		assert.Contains(t, err.Error(), "not connected")
	})
	...
	t.Run("successful send", func(t *testing.T) {
		...
		err := adapter.SendSubscribeTerminal(2, "pod-1", "ws://relay", "session-1", "token", true, 100)
		require.NoError(t, err)
	})
}
