/**
 * PodFile syntax highlighting via CodeMirror StreamLanguage.
 *
 * Tokenizes PodFile DSL for syntax coloring:
...
/** StreamLanguage tokenizer for PodFile */
const podfileTokenizer = {
  startState(): PodFileState {
    return { inHeredoc: false, heredocMarker: "" };
  },

  token(stream: StringStream, state: PodFileState): string | null {
    // Heredoc body
    if (state.inHeredoc) {
...
/** CodeMirror StreamLanguage for PodFile */
export const podfileLanguage = StreamLanguage.define(podfileTokenizer);
 ...
 */
const podfileHighlight = HighlightStyle.define([
...
/** Syntax highlighting extension — add this alongside podfileLanguage */
export const podfileSyntaxHighlighting = syntaxHighlighting(podfileHighlight);
