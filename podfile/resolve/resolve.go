// Package resolve injects final resolved CONFIG values into a PodFile AST.
// This is the bridge between the 3-layer config merge (defaults → user prefs → overrides)
// and the PodFile SSOT: resolved values are written back into ConfigDecl.Default
// so Runner eval reads them directly from the PodFile.
package resolve
...
// Used to identify which CONFIG fields were explicitly set by a user's PodFile Layer.
...
//  2. layerConfigNames — values explicitly set in user's PodFile Layer (already in merged AST)
...
func ResolveConfigValues(
	...
	// Append CONFIG declarations for systemOverrides not already in PodFile.
